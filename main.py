...
def get_team(session):
    ...
    user_id = _get_user_id(session)
    if not user_id: return RedirectResponse("/login", status_code=303)
    return page_layout("Team", "/team", session.get('user'), TeamPage(user_id=user_id, session=session), session=session)

@rt("/team/members", methods=["GET"])
def get_team_members(session):
    """HTMX endpoint: returns just the <tbody> rows for the members table."""
    from app.pages.team import _team_table_body
    user_id = _get_user_id(session)
    if not user_id: return ""
    active_org_id = session.get("active_org_id")
    if not active_org_id: return ""
    try:
...
def post_team_invite(session, invited_email: str, role: str, org_id: str):
    user_id = _get_user_id(session)
    if not user_id: return Div("Not authenticated.", cls="error-text")
    if not invited_email: return Div("Email address is required.", cls="error-text")
    ...
    try:
        ...
        result = auth_invite(
            email=invited_email,
            redirect_to=f"{APP_URL}/invite/accept?org={org_slug}"
        )
        invite_link = result.get("action_link", "")
        ...
        if email_sent:
            return Div(f"Invitation sent to {invited_email}.", cls="success-text")
        return Div(f"Membership created for {invited_email}. Email delivery not configured — share the invite link manually.", cls="success-text")
    except Exception as e:
        err = str(e)
        if "duplicate key" in err or "unique" in err.lower():
...
def post_team_role(session, membership_id: str, new_role: str):
    ...
    try:
        ...
        # Can't promote to admin unless owner
...
def post_team_remove(session, membership_id: str):
    ...
    try:
        ...
        # Don't allow admins to remove other admins
