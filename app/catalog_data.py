...
def seed_catalog():
    ...
    for d in DATASETS:
        row = dict(d)
        # Supabase expects JSON strings for jsonb/array columns via REST
        row["tags"] = d["tags"]  # text[] — pass as list, PostgREST handles it
        row["access_methods"] = d["access_methods"]
        row["schema_fields"] = d["schema_fields"]   # jsonb
        row["sample_rows"] = d["sample_rows"]        # jsonb
        payload.append(row)
